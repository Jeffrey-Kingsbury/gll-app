...
export default function ProjectEstimates({ projectId }) {
    ...
    if (estimates.length === 0) {
        return <div className="p-8 text-center text-stone-400 bg-stone-900 rounded-xl border border-stone-800">No estimates found for this project.</div>;
    }
        ...
        <div className="bg-[#1c1917] border border-stone-800 rounded-2xl overflow-hidden shadow-sm animate-in fade-in slide-in-from-bottom-4 duration-500">
            <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-stone-100 divide-stone-800/50">
                    {estimates.map(est => (
                        <tr key={est.internalid} className="hover:bg-stone-800 transition-colors group">
