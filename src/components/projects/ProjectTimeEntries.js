...
export default function ProjectTimeEntries({ projectId }) {
    ...
    if (entries.length === 0) {
        return <div className="p-8 text-center text-stone-400 bg-stone-900 rounded-xl border border-stone-800">No time entries found for this project.</div>;
    }

    return (
        <div className="bg-[#1c1917] border border-stone-800 rounded-2xl overflow-hidden shadow-sm animate-in fade-in slide-in-from-bottom-4 duration-500">
            <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-stone-100 divide-stone-800/50">
                    {entries.map(entry => (
                        <tr key={entry.internalid} className="hover:bg-stone-800 transition-colors">
                            <td className="px-6 py-4 text-stone-500 font-mono">{new Date(entry.date).toLocaleDateString()}</td>
                            <td className="px-6 py-4">
                                <div className="flex items-center gap-2">
                                    <div className="w-6 h-6 rounded-full bg-stone-200 bg-stone-700 flex items-center justify-center overflow-hidden">
                                        {entry.avatar_url ? (
                            ...
                            <td className="px-6 py-4">
                                <div className="text-white font-medium">{entry.task_name}</div>
                                <div className="text-stone-500 text-xs truncate max-w-[200px]">{entry.memo}</div>
                            </td>
                            <td className="px-6 py-4 text-center font-mono font-bold text-stone-300">{entry.hours}</td>
