          ...
          <table className="w-full text-left text-sm text-[#eaddcf]">
            <thead className="bg-stone-800 border-b border-stone-700">
              <tr>
                ...
                {/* Updated sort columns to match mapped names if needed, or keep generic keys mapped in action */}
                ...
                <Header label="Date Created" col="date" current={sortCol} dir={sortDir} onSort={handleSort} />
                ...
                estimates.map((est) => (
                  <tr key={est.internalid} className="hover:bg-stone-800/50 transition-colors group">
                    ...
                    <td className="px-6 py-4 font-medium text-[#eaddcf] group-hover:text-white transition-colors">
                      {/* FIX: Use project_name from DB */}
                      {est.project_name || est.project}
                    </td>
                    <td className="px-6 py-4 text-stone-400">
                      {/* FIX: Use client_name from DB */}
                      {est.client_name || est.client}
                    </td>
                    <td className="px-6 py-4 text-stone-500 text-xs">
                      {est.date ? new Date(est.date).toLocaleDateString() : '-'}
                    </td>
                    <td className="px-6 py-4 text-right font-mono text-amber-500">
                      {/* FIX: Use grand_total from DB */}
                      ${Number(est.grand_total || est.total || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                    </td>
                    <td className="px-6 py-4 text-right">
                      <span className={`px-2 py-1 rounded text-[10px] uppercase font-bold border ${est.status === 'Sent' ? 'bg-blue-900/30 text-blue-300 border-blue-800' :
