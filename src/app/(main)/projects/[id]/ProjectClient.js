...
import {
    ...
    PieChart, FileCheck2 // <-- Added FileCheck2 for Invoices icon
} from "lucide-react";
...
export default function ProjectClient({ data }) {
    ...
    // Destructure the new invoices array
    const { project, estimates, timeEntries, expenseReports = [], invoices = [] } = data;
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in pb-20">
                    ...
                    <div className="flex-1">
                        ...
                        {isEditing ? (
                            <input
                                ...
                                className="text-4xl font-bold text-white font-serif bg-transparent border-b-2 border-amber-500/50 focus:border-amber-500 outline-none w-full max-w-lg"
                            />
                        ) : (
                            <h1 className="text-4xl font-bold text-white font-serif">{project.name}</h1>
                        )}
                        ...
                        <div className="flex gap-2">
                            {isEditing ? (
                                ...
                                <button onClick={() => setIsEditing(true)}
                                    className="flex items-center gap-2 px-4 py-2 text-sm font-bold bg-stone-800 border border-stone-200 border-stone-700 text-stone-300 hover:border-amber-500 hover:text-amber-600 rounded-lg transition-all shadow-sm"
                                >
                            ...
                            <div>
                                ...
                                {isEditing ? (
                                    <input
                                        ...
                                        className="block w-32 text-xl font-mono font-bold text-white bg-transparent border-b border-stone-300 focus:border-amber-500 outline-none"
                                    />
                                ) : (
            ...
            {/* --- TABS --- */}
            {/* Added Invoices tab here */}
            <div className="flex gap-1 bg-stone-900 p-1 rounded-xl w-full md:w-fit flex-wrap">
                <TabButton id="details" label="Details" icon={<Layout size={16} />} active={activeTab} onClick={setActiveTab} />
                <TabButton id="budgets" label="Expense Reports" icon={<PieChart size={16} />} count={expenseReports.length} active={activeTab} onClick={setActiveTab} />
                <TabButton id="invoices" label="Invoices" icon={<FileCheck2 size={16} />} count={invoices.length} active={activeTab} onClick={setActiveTab} />
            ...
            <div className="min-h-[400px]">
                ...
                {activeTab === "details" && (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="md:col-span-2 bg-[#1c1917] rounded-2xl border border-stone-800 p-8 shadow-sm relative">
                            <h3 className="text-lg font-bold text-white mb-6">Project Overview</h3>
                            <div className="space-y-6">
                                ...
                                <div className="grid grid-cols-2 gap-6">
                                    <div>
                                        ...
                                        {isEditing ? (
                                            <input type="date" value={formData.start_date} onChange={e => setFormData({ ...formData, start_date: e.target.value })} className="w-full p-2 bg-stone-50 border border-stone-200 rounded-lg" />
                                        ) : (
                                    ...
                                    <div>
                                        ...
                                        {isEditing ? (
                                            <input type="date" value={formData.deadline} onChange={e => setFormData({ ...formData, deadline: e.target.value })} className="w-full p-2 bg-stone-50 border border-stone-200 rounded-lg" />
                                        ) : (
                                            <div className="flex items-center gap-2 text-stone-200 font-medium">
                        ...
                        <div className="bg-stone-900/50 rounded-2xl border border-stone-800 p-6 h-fit">
                            <h3 className="text-sm font-bold text-white uppercase tracking-wider mb-4">Client Information</h3>
                            <div className="space-y-4">
                                <div className="flex items-start gap-3">
                                    <Briefcase className="mt-1 text-stone-400" size={16} />
                                    <div>
                                        <div className="font-bold text-stone-200">{project.company_name}</div>
                                ...
                                {project.client_phone && (
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-sm text-stone-300">{project.client_phone}</span>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <table className="w-full text-left text-sm">
                            <thead className="bg-stone-900/50 text-stone-400 font-bold uppercase text-xs border-b border-stone-800">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 text-right">Total Est. Labor Budget</th>
                                    <th className="px-6 py-4 w-10"></th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-stone-100 divide-stone-800/50">
                                {expenseReports.length === 0 ? (
                                    <tr><td colSpan="5" className="px-6 py-12 text-center text-stone-400">No expense reports (budgets) found. Approve an estimate to generate one.</td></tr>
                                ) : expenseReports.map(report => (
                                    <tr key={report.internalid} className="hover:bg-stone-900/30 transition-colors group">
                                        <td className="px-6 py-4 text-stone-500 font-mono">{new Date(report.created_at).toLocaleDateString()}</td>
                                        <td className="px-6 py-4 font-bold text-white flex items-center gap-2">
                                            <PieChart size={16} className="text-amber-500" /> {report.name}
                                        </td>
                                        <td className="px-6 py-4 text-center">
                                            <span className="bg-amber-100 text-amber-700 px-2 py-1 rounded text-xs font-bold uppercase tracking-wider">{report.status}</span>
                                        </td>
                                        <td className="px-6 py-4 text-right font-mono font-medium">${parseFloat(report.total_budget || 0).toLocaleString()}</td>
                                        <td className="px-6 py-4 text-right">
                                            <Link href={`/expense-reports/${report.internalid}`} className="text-amber-600 hover:underline font-bold text-xs">Manage</Link>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                ...
                {activeTab === "invoices" && (
                    <div className="bg-[#1c1917] border border-stone-800 rounded-2xl overflow-hidden shadow-sm">
                        <table className="w-full text-left text-sm">
                            ...
                            <tbody className="divide-y divide-stone-100 divide-stone-800/50">
                                {invoices.length === 0 ? (
                                    <tr><td colSpan="5" className="px-6 py-12 text-center text-stone-400">No invoices generated for this project yet.</td></tr>
                                ) : invoices.map(invoice => (
                                    <tr key={invoice.internalid} className="hover:bg-stone-900/30 transition-colors group">
                                        <td className="px-6 py-4 font-bold text-white flex items-center gap-2">
                                            <FileCheck2 size={16} className="text-amber-500" /> {invoice.invoice_number}
                                        </td>
                                        <td className="px-6 py-4 text-stone-500 font-mono">{new Date(invoice.issue_date).toLocaleDateString()}</td>
                                        <td className="px-6 py-4 text-right font-mono font-medium">${parseFloat(invoice.total_amount || 0).toLocaleString()}</td>
                                        <td className="px-6 py-4 text-center">
                                            <span className={`px-2 py-1 rounded text-xs font-bold uppercase tracking-wider border ${getInvoiceStatusStyle(invoice.status)}`}>
                                                {invoice.status}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 text-right">
                                            <Link href={`/invoices/${invoice.internalid}`} className="text-amber-600 hover:underline font-bold text-xs">View</Link>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                        ...
                        <table className="w-full text-left text-sm">
                            ...
                            <tbody className="divide-y divide-stone-100 divide-stone-800/50">
                                {timeEntries.length === 0 ? (
                                    ...
                                    <tr key={entry.internalid} className="hover:bg-stone-900/30 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="font-medium text-stone-300">{entry.employee_name}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="font-medium text-white">{entry.task_name}</div>
                        ...
                        <table className="w-full text-left text-sm">
                            ...
                            <tbody className="divide-y divide-stone-100 divide-stone-800/50">
                                {estimates.length === 0 ? (
                                    ...
                                    <tr key={est.internalid} className="hover:bg-stone-900/30 transition-colors group">
                                        <td className="px-6 py-4 text-stone-500 font-mono">{new Date(est.created_at).toLocaleDateString()}</td>
                                        <td className="px-6 py-4 font-bold text-white">{est.project_name || "Untitled Estimate"}</td>
    ...
    return (
        <button
            ...
            className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg text-sm font-bold transition-all duration-200 ${isActive
                ...
                : "text-stone-500 hover:text-stone-700 hover:text-stone-300 hover:bg-stone-200/50 hover:bg-stone-800"
                }`}
        >
            {icon}
            <span>{label}</span>
            {count !== undefined && (
                <span className={`ml-1 text-xs px-1.5 py-0.5 rounded-full ${isActive ? 'bg-stone-100 bg-stone-800' : 'bg-stone-800 text-stone-500'}`}>
                    {count}
                </span>
            )}
        </button>
