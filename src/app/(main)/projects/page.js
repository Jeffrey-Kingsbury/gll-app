        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-stone-200 dark:border-stone-800 pb-6">
                ...
                <button
                    ...
                    className="flex items-center gap-2 bg-stone-900 dark:bg-amber-600 hover:bg-stone-800 dark:hover:bg-amber-500 text-white px-5 py-2.5 rounded-xl font-bold shadow-lg transition-all active:scale-95"
                >
            ...
            {isLoading ? (
                ...
                <div className="text-center py-20 bg-stone-50 dark:bg-stone-900/50 rounded-2xl border border-dashed border-stone-300 dark:border-stone-800">
                    <Briefcase className="mx-auto text-stone-400 mb-4" size={48} />
                    <h3 className="text-lg font-bold text-stone-600 dark:text-stone-300">No Projects Found</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {projects.map(project => (
                        <div key={project.internalid} className="bg-white dark:bg-[#1c1917] border border-stone-200 dark:border-stone-800 rounded-xl p-6 shadow-sm hover:shadow-md transition-all group relative">
                            ...
                            <div className="grid grid-cols-2 gap-4 text-sm border-t border-stone-100 dark:border-stone-800 pt-4">
                                <div>
                                    <span className="block text-stone-400 text-xs font-bold uppercase">Budget</span>
                                    <span className="font-mono font-medium dark:text-stone-300">${project.budget?.toLocaleString() || '0.00'}</span>
                                </div>
                                <div>
                                    <span className="block text-stone-400 text-xs font-bold uppercase">Estimates</span>
                                    <span className="font-mono font-medium dark:text-stone-300">{project.estimate_count} linked</span>
                                </div>
                            </div>
                        </div>
                    ))}
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in">
                    <div className="bg-white dark:bg-[#1c1917] w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 border border-stone-200 dark:border-stone-800">
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            <div>
                                ...
                                <input name="name" required className="w-full bg-stone-50 dark:bg-stone-900 border border-stone-200 dark:border-stone-700 rounded-lg p-2.5 outline-none focus:border-amber-500 dark:text-white" placeholder="e.g. Smith Kitchen Reno" />
                            </div>

                            <div>
                                <label className="block text-xs font-bold text-stone-400 uppercase mb-1">Customer</label>
                                <select name="customer_id" required className="w-full bg-stone-50 dark:bg-stone-900 border border-stone-200 dark:border-stone-700 rounded-lg p-2.5 outline-none focus:border-amber-500 dark:text-white appearance-none">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" name="budget" className="w-full bg-stone-50 dark:bg-stone-900 border border-stone-200 dark:border-stone-700 rounded-lg p-2.5 outline-none focus:border-amber-500 dark:text-white" placeholder="0.00" />
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-stone-400 uppercase mb-1">Status</label>
                                    <select name="status" className="w-full bg-stone-50 dark:bg-stone-900 border border-stone-200 dark:border-stone-700 rounded-lg p-2.5 outline-none focus:border-amber-500 dark:text-white">
