...
export default async function EmployeesPage() {
  ...
  const accessLevels = await getAccessLevelOptions();
              ...
              {employees && employees.length > 0 ? (
                employees.map((emp) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${emp.level_access === 3 ? 'bg-red-900/30 text-red-400 border border-red-900/50' :
                        emp.level_access === 2 ? 'bg-amber-900/30 text-amber-400 border border-amber-900/50' :
                          'bg-stone-800 text-stone-400 border border-stone-700'
                        }`}>
                        {accessLevels.find(l => l.internalid == emp.level_access)?.access_level_name || "Unknown Role"}
                      </span>
                    </td>
